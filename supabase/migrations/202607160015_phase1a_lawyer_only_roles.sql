-- Firm administration is not automatically legal authority.
-- Only lawyer-position roles may review conflict candidates, decide conflicts, or receive lawyer appointments.
...
begin
  ...
  if v_firm_id is null or not public.has_firm_role(
    v_firm_id,
    array['managing_partner', 'partner', 'associate']::public.membership_role[]
  ) then
...
as $$
  ...
  where fm.firm_id = p_firm_id
    ...
    and fm.role in ('managing_partner', 'partner', 'associate')
